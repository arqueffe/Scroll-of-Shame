...
CURRENT_BUILD=$(grep "^version:" "$PUBSPEC_FILE" | sed -E 's/^version: [0-9]+\.[0-9]+\.[0-9]+\+([0-9]+)$/\1/')

# If extraction failed (no build number found or empty result), default to 0 so increment gives us 1
